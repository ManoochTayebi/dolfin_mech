    ...
    def __init__(self,
        ...
        if (decoup):
            if   (self.kinematics.dim == 2):
                ...
                self.Psi_old      =   self.C1 * (self.kinematics.J_old**(-2/3) * (self.kinematics.IC_old + 1) - 3) # MG20200206: Plane strain, written with IC_2D
                self.Sigma_old    = 2*self.C1 * self.kinematics.J_old**(-2/3) * (self.kinematics.I - (self.kinematics.IC_old + 1)/3 * self.kinematics.C_inv_old)
                ...
                self.Sigma_old = 2*self.C1 * self.kinematics.J_old**(-2/3) * (self.kinematics.I - self.kinematics.IC_old/3 * self.kinematics.C_inv_old)
            ...
            if   (self.kinematics.dim == 2):
                ...
                self.Psi_old      =   self.C1 * (self.kinematics.IC_old - 2 - 2*dolfin.ln(self.kinematics.J_old)) # MG20200206: Plane strain
                self.Sigma_old    = 2*self.C1 * (self.kinematics.I - self.kinematics.C_inv_old)
